...
function SongViewer() {
  ...
  const handleChangeImage = async (event) => {
    ...
    const fileref = storageReferenceForAudio.child(file.name)
    await storageReferenceForAudio.put(imageFile)
    setCoverPhotoRef(await fileref.getDownloadURL())
...
const unSubmit = (event) => {
  ...
  db.collection("songs").doc(title).set({
    title: title,
